...
shinyServer(function(input, output) {
 ...
 #BUILDING TREE MAPS##################################
  ...
  #summarize(count=n())
  #billboarder() %>% 
   #bb_piechart(data = pie1)%>% bb_legend(show = FALSE)%>% bb_title(text = "Aisle Level Comparision", position = "center")
 ...
 # })
  ...
  #output$fourth <- renderBillboarder({
    #leastseller<-insta%>%
     ...
     # head(15)
    #billboarder() %>% 
      #bb_donutchart(data = leastseller)%>% bb_legend(show = FALSE)
  #})
 ...
 #itemFrequencyPlot(transactions,topN=20,type="absolute", col=brewer.pal(8,'Pastel2'), 
  ...
  #rules_t <- apriori(tr, 
                    # parameter = list(support = 0.0001, confidence = 0.0001, maxlen=3, ext=TRUE ), 
